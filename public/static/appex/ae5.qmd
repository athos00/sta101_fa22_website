...
:::{.callout-note}
The columns we *join by* must match the names of columns in each data frame. However the columns do not have to have the same name.
:::
...
```{r load-data}
set.seed(10)
flights2 = slice_sample(flights, n = 10000) %>%
    select(year, time_hour, dep_delay, arr_delay, origin, dest, tailnum, carrier)
flights2
...
Join `flights` and `airlines` and save the resulting data frame as `flights2`.
...
Create a new data set `airline_delays` that reports the average arrival delay for each carrier. Show only the 5 carrier airline with the worst arrival delays on average. No joins required here.
...
Create a new data set called `dest_delays` that reports the median arrival delay at each airport.
