...
7. Let $X \sim N(0, 3)$ and $Y \sim N(1, 2)$ where $X \sim N(\mu, \sigma)$ means "X is normally distributed with mean $\mu$ and standard deviation $\sigma$".

- What is the distribution of $X + Y$? 
- What is the distribution of $\frac{1}{2}X - Y$? 

8. Plot a histogram of the `num_lakes` from exercise 3. Overlay the plot with a normal distribution with  appropriate mean $\mu$ and standard deviation $\sigma$ to approximate the histogram. If $X \sim N(\mu, \sigma)$, what is $p(X \leq x)$? Recall that we define lower-case $x$ in exercise 3.
